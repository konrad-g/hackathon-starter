...
let linkedin = require('node-linkedin');
let clockwork = require('clockwork');
let paypal = require('paypal-rest-sdk');
let lob = require('lob');
let ig = bluebird.promisifyAll(require('instagram-node').instagram());
let foursquare = require('node-foursquare');

export class ApiController {

    constructor() {
        stripe = stripe(process.env.STRIPE_SKEY);
        twilio = twilio(process.env.TWILIO_SID, process.env.TWILIO_TOKEN);
        linkedin = linkedin(process.env.LINKEDIN_ID, process.env.LINKEDIN_SECRET, process.env.LINKEDIN_CALLBACK_URL);
        clockwork = clockwork({key: process.env.CLOCKWORK_KEY});
        lob = lob(process.env.LOB_KEY);
    ...
    getLastfm(req, res, next) {
        ...
        Promise.all([
            ...
            .then((results) => {
                ...
                let artistInfo: any = results[0];
                let artistTopAlbums: any = results[1];
                let artistTopTracks: any = results[2];
    ...
    getSteam(req, res, next) {
        ...
        Promise.all([
            ...
            .then((results) => {
                ...
                let playerAchievements: any = results[0];
                let playerSummaries: any = results[1];
                let ownedGames: any = results[2];
     ...
     */
    getLinkedin(req, res, next) {
        const token = req.user.tokens.find(token => token.kind === 'linkedin');
        const linkedinConnection = linkedin.init(token.accessToken);
        linkedinConnection.people.me((err, $in) => {
            if (err) {
                return next(err);
            }
